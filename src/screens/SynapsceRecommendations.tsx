...
import { Patient, Practitioner, ComplementaryApproach, Source, SynapsceData } from '@/data/synapsce_data';
...
const SynapsceRecommendations: React.FC<SynapsceRecommendationsProps> = ({ patient, data, additionalNotes, onBack }) => {
  ...
  const getSourceIcon = (type: string) => {
    ...
    } else if (type.toLowerCase().includes('fiche') || type.toLowerCase().includes('article')) {
  ...
  const getRelevantPractitioners = (): Practitioner[] => {
    const relevantApproachIds = relevantApproaches.map(approach => approach.id);
    
    return data.practitioners.filter(practitioner => 
      (practitioner.locationCity === 'Lausanne' || practitioner.locationCity === 'Pully') &&
      ...
      <header className="synapsce-header p-4 flex justify-between items-center shadow-md">
        <div className="flex items-center gap-3">
          <Button 
            ...
            onClick={onBack}
          >
            <ArrowLeft className="h-4 w-4 text-synapsce" />
          </Button>
          <SynapsceLogoSmall />
        </div>
        <div></div>
      ...
      <main className="container mx-auto py-8 px-4">
        <div className="mb-8">
          <h1 className="text-3xl font-semibold mb-2 text-synapsce">
            Recommandations SYNAPSCE pour : {patient.primaryDiagnosis} (Douleur, Sommeil, Anxiété)
          </h1>
        ...
        <Tabs defaultValue="approaches" className="w-full">
          <TabsList className="mb-6">
            <TabsTrigger value="approaches" className="px-6">Approches Complémentaires</TabsTrigger>
            <TabsTrigger value="resources" className="px-6">Ressources</TabsTrigger>
            <TabsTrigger value="practitioners" className="px-6">Professionnels</TabsTrigger>
          </TabsList>
                ...
                <Card key={approach.id} className="synapsce-card h-full">
                  <CardHeader className="pb-2">
                    <div className="flex justify-between items-start">
                      <CardTitle className="text-lg text-synapsce">{approach.name}</CardTitle>
                      <Badge variant="outline" className="bg-blue-50 text-synapsce">
                  ...
                  <CardContent>
                    <div className="mb-4">
                      <h4 className="font-medium mb-1 text-gray-700">Pertinence Fibromyalgie</h4>
                      <p className="text-sm">{getPertinenceText(approach.id)}</p>
                    </div>
                    
                    <Separator className="my-3" />
                    
                    <div>
                      <h4 className="font-medium mb-2 text-gray-700">En savoir plus</h4>
                      <div className="space-y-2">
                        {getSourcesForApproach(approach.id).map((source) => (
                          ...
                          >
                            <a href="#" className="flex items-center gap-2">
                              ...
                              <span className="truncate">{source.title.length > 40 
                        ...
                        {getSourcesForApproach(approach.id).length === 0 && (
                          <p className="text-sm text-gray-500">Pas de sources spécifiques disponibles</p>
                        )}
                      </div>
                    </div>
                  </CardContent>
          ...
          <TabsContent value="resources">
            <Card>
              <CardHeader>
                <CardTitle className="text-synapsce">Ressources et informations de confiance</CardTitle>
                <CardDescription>
                  Sélection de sources pertinentes pour la fibromyalgie et les approches complémentaires
                </CardDescription>
              </CardHeader>
              <CardContent>
                      ...
                      <CardHeader className="pb-2">
                        <div className="flex gap-3 items-center">
                          ...
                          <div>
                            ...
                            <CardDescription>{source.type} - {source.authorOrOrganization}</CardDescription>
                          </div>
                        </div>
                      </CardHeader>
                      <CardContent>
                  ...
                  <Card className="synapsce-card">
                    <CardHeader className="pb-2">
                      <div className="flex gap-3 items-center">
                        ...
                        <div>
                          <CardTitle className="text-base">{data.sources[7].title}</CardTitle>
                          <CardDescription>{data.sources[7].type} - {data.sources[7].authorOrOrganization}</CardDescription>
                        </div>
                      </div>
                    </CardHeader>
                    <CardContent>
                      ...
                      >
                        <a href="#" className="flex items-center gap-2">
                          Regarder la vidéo ({data.sources[7].durationMinutes} min)
                        </a>
                      </Button>
                    </CardContent>
                  </Card>
          ...
          <TabsContent value="practitioners">
            <Card>
              <CardHeader>
                <CardTitle className="text-synapsce">Professionnels Recommandés (Région Lausanne)</CardTitle>
                <CardDescription>
                  Praticiens formés aux approches complémentaires pertinentes pour la fibromyalgie
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-6">
                  {getRelevantPractitioners().map((practitioner) => (
                    <Card key={practitioner.id} className="synapsce-card">
                      ...
                      <CardContent>
                        <div className="mb-3">
                          <h4 className="font-medium text-sm mb-1">Spécialisations</h4>
                        ...
                        <div className="mb-3">
                          <h4 className="font-medium text-sm mb-1">Note SYNAPSCE</h4>
